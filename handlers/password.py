...
from services.crypto import generate_password, analyze_password, build_alphabet
...
def build_options_keyboard(data: dict) -> InlineKeyboardMarkup:
    ...
    def flag(name, label):
        ...
        builder.row(InlineKeyboardButton(
            text=f"{mark} {label}",
            callback_data=ToggleCB(field=name).pack()
        ))
    ...
    builder.row(InlineKeyboardButton(text="🔐 Сгенерировать", callback_data=GenerateCB().pack()))
    return builder.as_markup()

@router.message(Command("password"))
async def password(message: types.Message, state: FSMContext):
    data = {
        "length": 16, "use_letters": True, "use_digits": True,
        "use_symbols": True, "exclude_similar": False,
    }
    await state.set_data(data)
    await state.set_state(PasswordGen.choosing_length)
    await message.answer("Выбери длину пароля:", reply_markup=build_length_keyboard(data))

@router.callback_query(LengthCB.filter())
async def on_length(callback: types.CallbackQuery, callback_data: LengthCB, state: FSMContext):
...
async def on_back(callback: types.CallbackQuery, state: FSMContext):
    data = await state.get_data()
    await state.set_state(PasswordGen.choosing_length)
    await callback.message.edit_text("Выбери длину пароля:", reply_markup=build_length_keyboard(data))
    await callback.answer()

@router.callback_query(ToggleCB.filter())
async def on_toggle(callback: types.CallbackQuery, callback_data: ToggleCB, state: FSMContext):
    data = await state.get_data()
    ...
    if not (data["use_letters"] or data["use_digits"] or data["use_symbols"]):
        ...
        await callback.answer("Должен остаться хотя бы один тип символов", show_alert=True)
        return
...
async def on_generate(callback: types.CallbackQuery, state: FSMContext):
    ...
    alphabet = build_alphabet(
        data["use_letters"], data["use_digits"], data["use_symbols"], data["exclude_similar"]
    )
